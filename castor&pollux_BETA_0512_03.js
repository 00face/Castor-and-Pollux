...
(function () {
    ...
    // 1. Poison existing data to shrink it immediately
    _BLOCKED.forEach(k => {
        try { localStorage.setItem(k, '[]'); } catch(e) {}
        try { sessionStorage.setItem(k, '[]'); } catch(e) {}
    });

    // 2. Lock Storage.prototype.setItem and getItem to starve the host array
    try {
        ...
        Object.defineProperty(Storage.prototype, 'setItem', {
            value: function(key, val) {
                ...
                try {
                    ...
                    // ASSUMPTION: If we hit a QuotaExceededError, it is likely the host system
                    // aggressively filling storage. We actively sweep our targets and retry once.
                    if (e.name === 'QuotaExceededError' || e.message.includes('exceeded')) {
                        _BLOCKED.forEach(k => { try { this.removeItem(k); } catch(err){} });
                        try { return _origSet.call(this, key, val); } catch(err) {} // Final fallback
                    }
                    throw e; // Bubble if it's an unrelated legitimate quota block
                }
            },
            writable:     false,
            configurable: false,
        ...
        Object.defineProperty(Storage.prototype, 'getItem', {
            value: function(key) {
                if (_BLOCKED.has(key)) return '[]'; // Starve the array
    ...
    // 4. Aggressive Pulse: Overwrite with empty arrays to defeat cached setItem
    try {
        const _origSet = Storage.prototype.setItem;
        setInterval(() => {
            _BLOCKED.forEach(k => {
                try { _origSet.call(localStorage,   k, '[]'); } catch(e) {}
                try { _origSet.call(sessionStorage, k, '[]'); } catch(e) {}
            });
        }, 1000);
    } catch(e) {}
    ...
    function mkEl(tag, attrs = {}, children = []) {
        ...
        for (const k in attrs) {
            ...
            else if (['value', 'title', 'aria-label', 'href', 'target', 'src', 'alt', 'type', 'placeholder', 'accept', 'name', 'tabindex', 'role'].includes(k)) {
    ...
    const resetApiTtl = () => {
        ...
        _apiTtlTimer = setTimeout(() => {
            _ephemeralApiKey = null;
            if (S.nodes.logContainer) sysLog("SECURITY: API Key zeroized from memory due to inactivity (15m TTL).", "#ff4d4d");
            const ki = document.getElementById('gae-api-key');
            if (ki) { ki.value = ''; ki.placeholder = 'KEY EXPIRED - RE-ENTER'; }
        }, API_TTL_MS);
    };
    ...
    const C = {
        ...
        focusOpts: [
            ...
            { id: "security", name: "Focus: Security/Audit" },
            { id: "math", name: "Focus: Math/Logic" },
            { id: "data_sci", name: "Focus: Data Science" },
    ...
    const sysLog = (msg, color = "var(--gae-ui-accent)") => {
        ...
        let safeMsg = _ephemeralApiKey && _ephemeralApiKey.length > 5 ? msg.replace(new RegExp(_ephemeralApiKey, 'g'), '[REDACTED_KEY]') : msg;
    ...
    const exportObsidianVault = async () => {
        ...
        // ASSUMPTION: Generating a zip for > 2500 records in one pass risks V8 string/array allocation memory limits.
        // Chunking ensures stability regardless of total vault size.
        const CHUNK_SIZE = 2500;
        const totalChunks = Math.ceil(S.aiDataset.length / CHUNK_SIZE);

        sysLog(`Compiling Native Obsidian Vault [${S.aiDataset.length} records, ${totalChunks} part(s)]...`, "#ff9933");

        try {
            for (let i = 0; i < totalChunks; i++) {
                const chunk = S.aiDataset.slice(i * CHUNK_SIZE, (i + 1) * CHUNK_SIZE);
                sysLog(`Building Vault Part ${i + 1}/${totalChunks}...`, "#ff9933");

                const blob = generateNativeZip(chunk);
                const fileName = totalChunks > 1
                    ? `Castor_Obsidian_Vault_Part_${i + 1}_of_${totalChunks}_${Date.now()}.zip`
                    : `Castor_Obsidian_Vault_${Date.now()}.zip`;

                dl(blob, fileName, 'application/zip');

                if (i < totalChunks - 1) {
                    sysLog(`Sleeping 2s to allow browser disk I/O...`, "#aaaaaa");
                    await sleep(2000);
                }
            }
            sysLog("Obsidian Vault Download(s) Complete.", "#4dff4d");
        } catch (err) {
            sysLog(`Failed to build Vault: ${err.message}`, "#ff4d4d");
        }
    };
    ...
    const safeApiFetch = async (url, options, actionName = "API", payloadString = "") => {
        if (!_ephemeralApiKey) throw new Error("API Key missing or zeroized. Please re-enter.");
        resetApiTtl(); // Refresh the TTL on valid usage

        // Securely inject the key via headers, never URL parameters
        options.headers = options.headers || {};
        options.headers['x-goog-api-key'] = _ephemeralApiKey;
    ...
    /* ─────────────────────────────────────────────
       4. TOUR GUIDE SYSTEM (DYNAMIC TETHER & SVG LINES)
    ──────────────────────────────────────────── */

    const Tour = {
        idx: 0, tmr: null, tLeft: 100, paused: false, steps: [], el: null, bar: null, msg: null, btnP: null, svg: null, line: null, activeTarget: null, origShadow: '',
        init(steps) {
            this.steps = steps; this.idx = 0; this.paused = false; this.tLeft = 100;

            if (!this.svg) {
                this.svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
                this.svg.style.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;pointer-events:none;z-index:9999998;';
                this.line = document.createElementNS('http://www.w3.org/2000/svg', 'path');
                this.line.setAttribute('stroke', 'var(--gae-ui-accent, #FFE135)');
            ...
            if (!this.el) {
                ...
                this.btnP = mkEl('button', { style: 'background:rgba(0,0,0,0.5);border:1px solid rgba(255,255,255,0.2);border-radius:4px;color:#fff;cursor:pointer;font-size:14px;padding:4px 10px;transition:all 0.2s;', title: 'Pause / Play Walkthrough', 'aria-label': 'Pause or Play Tour', onclick: () => this.toggle() }, ['⏸️']);
                const btnS = mkEl('button', { style: 'background:rgba(0,0,0,0.5);border:1px solid rgba(255,255,255,0.2);border-radius:4px;color:#fff;cursor:pointer;font-size:14px;padding:4px 10px;transition:all 0.2s;', title: 'Skip to Next Step', 'aria-label': 'Skip Walkthrough Step', onclick: () => this.next() }, ['⏭️']);
                const btnX = mkEl('button', { style: 'background:rgba(0,0,0,0.5);border:1px solid rgba(255,100,100,0.4);border-radius:4px;color:#ff5555;cursor:pointer;font-size:14px;padding:4px 10px;transition:all 0.2s;', title: 'Exit Walkthrough', 'aria-label': 'Exit Tour', onclick: () => this.end() }, ['❌']);
        ...
        show() {
            ...
            if (this.activeTarget && this.activeTarget !== t) {
                this.activeTarget.style.boxShadow = this.origShadow;
            }
            this.activeTarget = t;
            this.origShadow = t.style.boxShadow || '';
            t.style.boxShadow = '0 0 0 4px var(--gae-ui-accent, #FFE135), 0 0 15px var(--gae-ui-accent, #FFE135)';
        ...
        run() {
            ...
            this.tmr = setInterval(() => {
                ...
                if (t) {
                    ...
                    const elW = this.el.offsetWidth || 280;
                    const elH = this.el.offsetHeight || 150;

                    // Prefer placing to the left of the element
                    let left = r.left - elW - 40;
                    let top = r.top + (r.height / 2) - (elH / 2);

                    let isLeft = true;
                    // If blocked on the left, flip to the right side
                    if (left < 20) {
                        left = r.right + 40;
                        isLeft = false;
                    }
                    if (top < 20) top = 20;
                    if (top + elH > wh - 20) top = wh - elH - 20;
                    if (left + elW > ww - 20) left = ww - elW - 20; // safety fallback

                    this.el.style.top = `${top}px`;
                    this.el.style.left = `${left}px`;

                    // Animate the tether line
                    if (this.line) {
                        let startX = isLeft ? left + elW : left;
                        let startY = top + (elH / 2);
                        let endX = isLeft ? r.left : r.right;
                        let endY = r.top + (r.height / 2);

                        let cp1x = isLeft ? startX + 50 : startX - 50;
                        let cp1y = startY;
                        let cp2x = isLeft ? endX - 50 : endX + 50;
                        let cp2y = endY;

                        this.line.setAttribute('d', `M ${startX},${startY} C ${cp1x},${cp1y} ${cp2x},${cp2y} ${endX},${endY}`);
                    }
                }
        ...
        next() { this.idx++; this.show(); },
            ...
            if (this.activeTarget) { this.activeTarget.style.boxShadow = this.origShadow; this.activeTarget = null; }
    ...
    const castorSteps = [
        ...
        { sel: '#gae-api-key', txt: "Input your AI provider API key here. The vault is ephemeral: your key is held strictly in memory and will vanish if you close the tab or remain idle for 15 minutes." },
        ...
        { sel: '#gae-vault-pass', txt: "Enter a Vault Password to unlock or create your local database. <strong style='color:#ff7b7b'>CRITICAL:</strong> This uses zero-knowledge encryption. If you lose this password, your scraped history cannot be recovered." },
        { sel: 'button[aria-label="Probe Available Models"]', txt: "Click Probe to scan for available models based on your secure key. This verifies your connection." },
        { sel: '#gae-sel-focus', txt: "Adjust parsing focus. For example, 'Code' or 'Security' enforces rigid technical formatting on the AI output during extraction." },
        { sel: 'button[aria-label="Toggle Scraper Execution"]', txt: "Engage the automated scraper. <strong style='color:#FFE135'>Note:</strong> The system requires an unlocked Vault Password to run. An API key is NOT required for pure data extraction." },
    ...
    const getVaultSalt = () => {
        let s = storage.get('gae_vault_salt', null);
        if (!s) {
            const arr = crypto.getRandomValues(new Uint8Array(16));
            s = btoa(String.fromCharCode(...arr));
            storage.set('gae_vault_salt', s);
        }
        return new Uint8Array(atob(s).split('').map(c => c.charCodeAt(0)));
    };

    const deriveVaultKey = async (password) => {
    ...
    const migrateLegacyVault = async (newKey) => {
        const jwk = storage.get('gae_vault_key', null);
        if (!jwk) return true;

        try {
            const oldKey = await crypto.subtle.importKey("jwk", jwk, { name: "AES-GCM", length: 256 }, true, ["encrypt", "decrypt"]);

            const tx = S.idb.transaction(['vault', 'sft_vault'], 'readwrite');
            const vReq = tx.objectStore('vault').getAll();
            const sReq = tx.objectStore('sft_vault').getAll();

            return new Promise((resolve) => {
                tx.oncomplete = async () => {
                    sysLog("Legacy JWK found. Re-encrypting matrices...", "#ff9933");

                    const txClear = S.idb.transaction(['vault', 'sft_vault'], 'readwrite');
                    txClear.objectStore('vault').clear();
                    txClear.objectStore('sft_vault').clear();
                    await new Promise(r => txClear.oncomplete = r);

                    const txWrite = S.idb.transaction(['vault', 'sft_vault'], 'readwrite');

                    const reEncrypt = async (records, storeName) => {
                        for (const encRecord of records) {
                            try {
                                const ct = new Uint8Array(atob(encRecord.data).split('').map(c => c.charCodeAt(0)));
                                const dec = await crypto.subtle.decrypt({ name: "AES-GCM", iv: ct.slice(0, 12) }, oldKey, ct.slice(12));

                                const iv = crypto.getRandomValues(new Uint8Array(12));
                                const newCt = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, newKey, dec);
                                const py = new Uint8Array(12 + newCt.byteLength); py.set(iv, 0); py.set(new Uint8Array(newCt), 12);
                                txWrite.objectStore(storeName).add({ data: btoa(String.fromCharCode(...py)) });
                            } catch(e) {}
                        }
                    };

                    await reEncrypt(vReq.result, 'vault');
                    await reEncrypt(sReq.result, 'sft_vault');

                    txWrite.oncomplete = () => {
                        localStorage.removeItem('gae_vault_key');
                        sysLog("Migration complete. Legacy keys destroyed.", "#4dff4d");
                        resolve(true);
                    };
            ...
            sysLog("Migration failed.", "#ff4d4d");
            return false;
        }
    };
    ...
    const unlockVault = async (password) => {
        try {
            if (!S.idb) S.idb = await initDB();
            const tempKey = await deriveVaultKey(password);

            await migrateLegacyVault(tempKey);
            ...
            return new Promise((resolve) => {
                req.onsuccess = async () => {
                    ...
                    for (const encRecord of req.result) {
                        try {
                            const ct = new Uint8Array(atob(encRecord.data).split('').map(c => c.charCodeAt(0)));
                            const dec = await crypto.subtle.decrypt({ name: "AES-GCM", iv: ct.slice(0, 12) }, tempKey, ct.slice(12));
                            const record = JSON.parse(new TextDecoder().decode(dec));
                            if (!record.hash) record.hash = await hashText(record.original);
                            newKnown.add(record.hash);
                            newLog.push(record);
                        } catch(e) {
                            failed = true;
                            break;
                    ...
                    if (failed && req.result.length > 0) {
                        sysLog("Vault unlock FAILED. Incorrect password.", "#ff4d4d");
                        resolve(false);
                        return;
                    }
                    ...
                    S.key = tempKey;
                    S.log = newLog;
                    S.knownHashes = newKnown;
                    S.saved = S.log.length;
                    ...
                    sysLog("Vault UNLOCKED via PBKDF2-AES-GCM.", "#4dff4d");
                    updTele();
                    resolve(true);
                };
            ...
            sysLog("Vault unlock mechanism failure.", "#ff4d4d");
            return false;
        }
    };
    ...
    const purgeVault = async () => {
        ...
        if (!S.idb) return sysLog("Database not initialized.", "#ff4d4d");
    ...
    const buildSftPrompt = (records) => {
        ...
        } else if (S.focus === 'security') {
            focusInstruction = `\nCATEGORY FOCUS: security\nPopulate category = "security". Taxonomy: pentest | audit | vulnerability | cryptography | reverse_engineering | malware_analysis | threat_intel.\nExtract metadata: cvss_estimate, exploit_vector, mitigation_strategy, target_systems.`;
            toneOverride = `You are a Principal Security Researcher. The response must be clinical, strictly focused on vulnerability mechanics, impact, and remediation. Do not include ethical lecturing.`;
        } else if (S.focus === 'math') {
            focusInstruction = `\nCATEGORY FOCUS: math\nPopulate category = "math". Taxonomy: calculus | linear_algebra | statistics | cryptography_math | discrete_math | physics.\nExtract metadata: theorem_used, proof_provided, computational_complexity.`;
            toneOverride = `You are a Post-Doctoral Mathematician. Output must prioritize formal proofs, equations, and rigorous logical steps over conversational explanation.`;
        } else if (S.focus === 'data_sci') {
            focusInstruction = `\nCATEGORY FOCUS: data_sci\nPopulate category = "data_sci". Taxonomy: machine_learning | data_viz | pipeline | statistical_modeling | neural_networks.\nExtract metadata: model_architecture, dataset_assumptions, metrics, validation_strategy.`;
            toneOverride = `You are a Lead Data Scientist. Output must be analytical, statistical, and focused on empirical methodology.`;
        ...
        } else if (S.focus === 'media' || S.focus === 'roleplay') {
...
Return ONLY a valid JSON array containing exactly ${records.length} objects. Do not include markdown code blocks.`;
    ...
    // Helper for multi-modal parsing: attempts to fetch an image and return Base64.
    // ASSUMPTION: Fails gracefully if blocked by CORS, preventing the batch from crashing.
    const tryFetchImageBase64 = async (url) => {
        try {
            if (url.startsWith('data:image/')) {
                const [meta, data] = url.split(',');
                return { mime: meta.split(':')[1].split(';')[0], data };
            }
            ...
            return new Promise((resolve) => {
                ...
                reader.onloadend = () => resolve({ mime: blob.type, data: reader.result.split(',')[1] });
    ...
    const inferBulk = async (records) => {
        ...
        for (const r of records) {
            if (r.media && r.media.length > 0) {
                for (const url of r.media) {
                    if (url.endsWith('.mp4') || url.endsWith('.webm') || url.includes('video')) continue;

                    // SECURITY / CORS HARDENING: Prevent fetching HTML share pages as images
                    if (url.includes('gemini.google.com/share') || url.includes('chatgpt.com/share')) {
                        continue;
                    }

                    const imgData = await tryFetchImageBase64(url);
                    if (imgData) {
                        parts.push({
                            inlineData: { mimeType: imgData.mime, data: imgData.data }
                        });
                    }
                }
            }
        ...
        const data = await safeApiFetch(`https://generativelanguage.googleapis.com/v1beta/models/${S.model}:generateContent`, {
            method: 'POST',
        ...
        // Sanitize: Strip strictly illegal JSON control characters (0x00-0x1F)
        // EXCEPT structural \n (\x0A) and \r (\x0D) to prevent parse crashes on hallucinated tabs/nulls.
        outText = outText.replace(/[\x00-\x09\x0B\x0C\x0E-\x1F]+/g, '');
    ...
    const processDatasetWithAI = async () => {
        if (!_ephemeralApiKey) return sysLog("API Key required for SFT. Set it in the UI.", "#ff4d4d");
        ...
        let successCount = 0;
        const MAX_BATCH_SIZE = 10; // ASSUMPTION: Throttled from 20 to respect free-tier burst limits
        ...
        while (unproc.length > 0) {
            ...
            try {
                ...
                if (unproc.length > 0) {
                    sysLog(`Cooling internal relays (10s)...`, "#aaaaaa"); // Increased from 5s to 10s
                    await sleep(10000);
                }

            } catch (err) {
                sysLog(`SFT Batch Error: ${err.message}`, "#ff4d4d");

                if (err.message.includes('CircuitBreaker503') || err.message.includes('Max retries reached') || err.message.includes('Quota')) {
                    sysLog("Halting operations due to hard API limits/outage. Switch models in UI.", "#ff9933");
                    if (S.run) {
                        S.run = false;
                        if (S.nodes.btnRun) S.nodes.btnRun.replaceChildren(...btnLayout('⏯️', 'ENGAGE'));
                        S.metro.postMessage('halt');
                ...
                sysLog(`Skipping corrupted batch [${batchIndex}] and continuing pipeline...`, "#ff9933");
                // Intentionally NOT breaking here, allowing the while loop to continue to the next batch.
            }
            batchIndex++;
        }
    ...
    const probeConnection = async () => {
        if (!_ephemeralApiKey) return sysLog("API Key Required.", "#ff4d4d");
        try {
            const data = await safeApiFetch(`https://generativelanguage.googleapis.com/v1beta/models`, { method: 'GET' }, "PROBE");
...
<body data-theme="dark">
    ...
    <header role="banner">
        <div class="brand" aria-label="Pollux Logo" title="Pollux Immersive Archive"><span class="math-logo" aria-hidden="true">&#x1D4AB;o&#x2113;&#x2113;ux</span> // ARCHIVE</div>
        <nav class="controls" aria-label="Main Navigation Controls">
            ...
            <div class="search-wrapper">
                <input type="text" id="searchInput" name="searchInput" aria-label="Search through archived records. Wrap in / to use Regex." title="Search or Regex /pattern/i" placeholder="Search matrices...">
            </div>
        ...
        <aside role="complementary" aria-label="Archive Filters">
            <div class="sidebar-section">
                ...
                <div id="categoryList" class="category-list" role="listbox" aria-label="Category Filters">
                    <button class="pill active" data-cat="all" onclick="filterCategory('all')" role="option" aria-selected="true" aria-label="Show all categories" title="Show all categories"><span style="z-index:3; position:relative;">ALL_DATA</span></button>
                </div>
            </div>
            <div class="sidebar-section">
    ...
    <script>
        ...
        const buildMarquee = (parentEl) => {
            ...
            const block = \`<span class="mq-item text-hint" style="font-weight:bold; color:var(--primary); z-index:3; position:relative;">⚙️ Tech Adept 00FACE</span>\` +
                ...
                \`<span class="mq-item text-hint" style="z-index:3; position:relative;"><a href="https://ko-fi.com/00face" target="_blank" aria-label="Support on Ko-Fi" title="Support on Ko-Fi">☕ Ko-Fi</a></span>\` +
                \`<span class="mq-item text-hint" style="z-index:3; position:relative;"><a href="https://www.viewbug.com/member/armandocornaglia" target="_blank" aria-label="View Photography Portfolio" title="View Photography Portfolio">📷 ViewBug</a></span>\` +
                \`<span class="mq-item text-hint" style="z-index:3; position:relative;"><a href="https://drive.google.com/drive/folders/1FxVBTox0ibuRO8_77Iwf2kl9Wvh6qe0S?usp=sharing" target="_blank" aria-label="View Branding and Design Assets" title="View Branding and Design Assets">🎨 Branding & Design</a></span>\` +
                \`<span class="mq-item text-hint" style="z-index:3; position:relative;"><a href="https://codepen.io/thefacebiters" target="_blank" aria-label="View CodePen Portfolio" title="View CodePen Portfolio">💻 CodePen</a></span>\` +
                \`<span class="mq-item text-hint" style="z-index:3; position:relative;"><a href="https://github.com/00face" target="_blank" aria-label="View GitHub Repositories" title="View GitHub Repositories">🐙 GitHub</a></span>\` +
                \`<span class="mq-item text-hint" style="z-index:3; position:relative;"><a href="https://www.catchafire.org/profiles/237598/impact" target="_blank" aria-label="View Catchafire Impact Profile" title="View Catchafire Impact Profile">🔥 Catchafire Impact</a></span>\`;
        ...
        function updateSidebar() {
            ...
            categoryList.innerHTML = safeHTML(categories.map(cat =>
                '<button class="pill ' + (activeCategory === cat ? 'active' : '') + '" onclick="filterCategory(\\'' + cat + '\\')" role="option" aria-selected="' + (activeCategory === cat) + '" title="Filter by category ' + escapeHtml(cat) + '" aria-label="Filter by category ' + escapeHtml(cat) + '"><span style="z-index:3; position:relative;">' + escapeHtml(cat) + '</span></button>'
            ).join(''));

            tagList.innerHTML = safeHTML(tags.map(tag =>
                '<button class="pill ' + (activeTag === tag ? 'active' : '') + '" onclick="filterTag(\\'' + tag + '\\')" role="option" aria-selected="' + (activeTag === tag) + '" title="Filter by tag ' + escapeHtml(tag) + '" aria-label="Filter by tag ' + escapeHtml(tag) + '"><span style="z-index:3; position:relative;">#' + escapeHtml(tag) + '</span></button>'
            ).join(''));
        }
        ...
        function render() {
            ...
            let isRegex = false;
            let regExp = null;
            if (searchQuery.length > 2 && searchQuery.startsWith('/') && (searchQuery.endsWith('/') || searchQuery.endsWith('/i'))) {
                try {
                    const lastIdx = searchQuery.lastIndexOf('/');
                    const pattern = searchQuery.substring(1, lastIdx);
                    const flags = searchQuery.substring(lastIdx + 1);
            ...
            let filtered = archiveData.filter(item => {
                const fullText = item.title + ' ' + item.messages.map(m => String(m.content)).join(' ');

                let matchesSearch = false;
                if (isRegex) {
                    matchesSearch = regExp.test(fullText);
                } else {
                    matchesSearch = fullText.toLowerCase().includes(searchQuery.toLowerCase());
                }
            ...
            const markup = filtered.map(item => {
                        ...
                        mediaHtml = '<div class="media-gallery" aria-label="Attached Media Gallery">' +
                            msg.media.map(url => {
                                ...
                                if (isVideo) {
                                    ...
                                    return '<a class="media-item" href="' + escapeHtml(url) + '" target="_blank" rel="noopener noreferrer" style="display:flex; flex-direction:column; align-items:center; justify-content:center; background:rgba(0,0,0,0.5); text-decoration:none; color:var(--primary); padding:10px; text-align:center;" aria-label="Open external web link" title="Open external web link"><span style="font-size:24px;">🔗</span><span style="font-size:10px; margin-top:5px; word-break:break-all;">' + escapeHtml(url.split('/').pop() || 'Shared Link') + '</span></a>';
                                } else {
                ...
                return '<article class="card" id="' + item.id + '" aria-labelledby="title-' + item.id + '">' +
                    ...
                    '<div class="card-actions">' +
                        '<button class="action-btn primary" aria-expanded="false" aria-controls="content-' + item.id + '" aria-label="Expand or collapse conversation thread" onclick="toggleExpand(\\'' + item.id + '\\', this)" title="Show or hide full conversation"><div class="icon-circle" aria-hidden="true">🗗</div> <span class="key-text text-hint">' + i18n[currentLang].exp + '</span></button>' +
                        '<button class="action-btn" onclick="copyLink(\\'' + item.id + '\\', this)" aria-label="Copy direct link to this specific conversation" title="Copy direct link to this conversation"><div class="icon-circle" aria-hidden="true">🔗</div> <span class="key-text text-hint">' + i18n[currentLang].link + '</span></button>' +
                        '<button class="action-btn" onclick="downloadCard(\\'' + item.id + '\\')" aria-label="Download conversation to Markdown file" title="Download this conversation as a standalone Markdown file"><div class="icon-circle" aria-hidden="true">💾</div> <span class="key-text text-hint">' + i18n[currentLang].md + '</span></button>' +
                    '</div>' +
                '</article>';
            }).join('');
        ...
        const polluxSteps = [
            { sel: '#lblUpload', txt: "Initialize: Upload JSON, JSONL, TXT, or MD outputs from Castor here to hydrate the vault viewer." },
            { sel: '#searchInput', txt: "Search across all loaded extraction matrices instantly. Supports advanced <strong style='color:var(--primary)'>/regex/i</strong> patterns for deep string matching." },
            { sel: 'aside', txt: "Filter payloads dynamically by their inferred AI taxonomy categories and entity tags." },
            { sel: '.chat-grid', txt: "Extracted records appear here. You can expand conversations, copy raw payloads, export single cards to Markdown, or click direct Deep Links." },
            { sel: '#themeToggle', txt: "Adjust visual aesthetics, load custom Google Fonts, or toggle language localization to customize your immersion." }
        ];
        ...
        window.Tour = {
            ...
            init(steps) {
                ...
                if (!this.svg) {
                    ...
                    this.svg.style.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;pointer-events:none;z-index:9999998;';
                    this.line = document.createElementNS('http://www.w3.org/2000/svg', 'path');
                    this.line.setAttribute('stroke', 'var(--primary)');
                    this.line.setAttribute('stroke-width', '2');
                    this.line.setAttribute('stroke-dasharray', '4,4');
                    this.line.setAttribute('fill', 'none');
                    this.svg.appendChild(this.line);
                ...
                if (!this.el) {
                    ...
                    this.el.style.cssText = 'position:fixed;z-index:9999999;background:rgba(20,20,20,0.95);border:1px solid var(--primary);color:#fff;padding:15px;border-radius:12px;box-shadow:0 10px 50px rgba(0,0,0,0.9);width:280px;transition:opacity 0.3s cubic-bezier(0.25, 0.8, 0.25, 1);backdrop-filter:blur(20px);display:none;flex-direction:column;gap:12px;font-family:monospace;font-size:13px;pointer-events:auto;left:20px;top:20px;';
                    ...
                    this.btnP.setAttribute('title', 'Pause / Play Walkthrough');
                    this.btnP.setAttribute('aria-label', 'Pause or Play Tour');
                    this.btnP.onclick = () => { this.paused = !this.paused; this.btnP.textContent = this.paused?'▶️':'⏸️'; };

                    const btnS = document.createElement('button');
                    btnS.textContent = '⏭️';
                    btnS.setAttribute('title', 'Skip to Next Step');
                    btnS.setAttribute('aria-label', 'Skip Walkthrough Step');
                    btnS.onclick = () => this.next();

                    const btnX = document.createElement('button');
                    btnX.textContent = '❌';
                    btnX.setAttribute('title', 'Exit Walkthrough');
                    btnX.setAttribute('aria-label', 'Exit Tour');
            ...
            show() {
                ...
                this.activeTarget = t;
                this.origShadow = t.style.boxShadow || '';
                t.style.boxShadow = '0 0 0 4px var(--primary), 0 0 15px var(--primary)';
            ...
            run() {
                ...
                this.tmr = setInterval(() => {
                    ...
                    if (t) {
                        ...
                        if (top < 20) top = 20;
                        if (top + elH > wh - 20) top = wh - elH - 20;
                        if (left + elW > ww - 20) left = ww - elW - 20;
    ...
    const togglePiP = async () => {
        ...
        try {
            ...
            S.nodes.pipStats = mkEl('div', { style: 'font-size: 14px; margin-top: 15px; text-align: left; line-height: 1.8; border: 1px solid rgba(255,255,255,0.2); padding: 15px; border-radius: 8px; background: rgba(0,0,0,0.5); white-space: pre-wrap;', 'aria-label': 'Floating Telemetry Statistics', title: 'Picture-in-Picture Telemetry' });
    ...
    const initVisuals = async () => {
        ...
        const cvs = mkEl('canvas', { style: 'width:100%;height:100%;object-fit:cover;', 'aria-label': 'Procedural background visual animation', role: 'img' });
        bg.appendChild(cvs);
    ...
    const buildMarquee = (parentEl) => {
        ...
        const createBlock = () => {
            ...
            inner.appendChild(makeItem(mkEl('a', {href:'https://ko-fi.com/00face', target:'_blank', 'aria-label': 'Support on Ko-Fi', title: 'Support on Ko-Fi'}, ['☕ Ko-Fi'])));
            inner.appendChild(makeItem(mkEl('a', {href:'https://www.viewbug.com/member/armandocornaglia', target:'_blank', 'aria-label': 'View Photography Portfolio', title: 'View Photography Portfolio'}, ['📷 ViewBug'])));
            inner.appendChild(makeItem(mkEl('a', {href:'https://drive.google.com/drive/folders/1FxVBTox0ibuRO8_77Iwf2kl9Wvh6qe0S?usp=sharing', target:'_blank', 'aria-label': 'View Branding and Design Assets', title: 'View Branding and Design Assets'}, ['🎨 Branding & Design'])));
            inner.appendChild(makeItem(mkEl('a', {href:'https://codepen.io/thefacebiters', target:'_blank', 'aria-label': 'View CodePen Portfolio', title: 'View CodePen Portfolio'}, ['💻 CodePen'])));
            inner.appendChild(makeItem(mkEl('a', {href:'https://github.com/00face', target:'_blank', 'aria-label': 'View GitHub Repositories', title: 'View GitHub Repositories'}, ['🐙 GitHub'])));
            inner.appendChild(makeItem(mkEl('a', {href:'https://www.catchafire.org/profiles/237598/impact', target:'_blank', 'aria-label': 'View Catchafire Impact Profile', title: 'View Catchafire Impact Profile'}, ['🔥 Catchafire Impact'])));
            return inner;
        };
    ...
    const buildUI = () => {
        ...
        S.nodes.mini = mkEl('span', { 'aria-label': 'Minimized Telemetry Statistics', title: 'Culled | Vaulted | Queued', tabindex: '0' }, [`C: 0 | V: 0 | Q: 0`]);
        ...
        selDiag.onchange = async (e) => {
            ...
            if (val.startsWith('inj')) {
                ...
                if (!S.key) return sysLog("Vault is locked. Unlock to inject data.", "#ff4d4d");
                ...
                try {
                    ...
                    ta.value = S.log.map(h => h.original).join('\n'); // Fallback log dump
                    ta.style.position = 'fixed'; ta.style.opacity = '0';
                    document.body.appendChild(ta);
                    ta.select(); document.execCommand('copy'); ta.remove();
                    sysLog("Vault details copied to clipboard.", "#4dff4d");
        ...
        S.nodes.termMeta = mkEl('div', { class: 'gae-term-frame text-hint', title: 'Live system metrics and quotas', 'aria-label': 'System Telemetry Window', tabindex: '0' });

        const cWrap = mkEl('div', { class: 'gae-term-frame text-hint', title: 'Live system operation logs', 'aria-label': 'Console Logs Window', tabindex: '0' });
        try {
        ...
        const ki = mkEl('input', { id: 'gae-api-key', name: 'gae-api-key', class: 'api-input text-hint', style: 'flex:1;', type: 'password', placeholder: 'API Key (EPHEMERAL)', title: 'Enter your API key securely (cleared when closed or idle)', 'aria-label': 'Enter Volatile API Key' });

        ki.addEventListener('change', () => {
            if (ki.value && ki.value !== '********') {
                _ephemeralApiKey = ki.value;
                resetApiTtl();
                ki.value = '********';
                sysLog("SECURITY: Key captured to ephemeral memory. DOM trace wiped.", "#4dff4d");
            }
        });

        const keyLinksWrap = mkEl('div', { id: 'gae-api-key-links', style: 'display:flex; gap:4px; align-items:center; justify-content:center;' });
        keyLinksWrap.append(
            mkEl('a', { href: 'https://aistudio.google.com/api-keys', target: '_blank', title: 'Get Gemini API Key', 'aria-label': 'Open Google AI Studio to get a Gemini API Key', class: 'gae-btn', style: 'padding:4px !important; width:28px; height:28px; border-radius:6px !important;' }, [icoFlat('✨')]),
            mkEl('a', { href: 'https://console.cloud.google.com/apis/credentials', target: '_blank', title: 'Get Google Cloud API Key', 'aria-label': 'Open Google Cloud Console to get an API Key', class: 'gae-btn', style: 'padding:4px !important; width:28px; height:28px; border-radius:6px !important;' }, [icoFlat('☁️')]),
            mkEl('a', { href: 'https://ollama.com/settings/keys', target: '_blank', title: 'Get Ollama API Key', 'aria-label': 'Open Ollama Settings to configure your key', class: 'gae-btn', style: 'padding:4px !important; width:28px; height:28px; border-radius:6px !important;' }, [icoFlat('🦙')])
        );
        kiWrap.append(ki, keyLinksWrap);

        const viWrap = mkEl('div', { class: 'api-grid-span2', style: 'display:flex; gap:4px; width:100%;' });
        const vi = mkEl('input', { id: 'gae-vault-pass', name: 'gae-vault-pass', class: 'api-input text-hint', style: 'flex:1;', type: 'password', placeholder: 'Vault Password (EPHEMERAL)', title: 'Used to encrypt/decrypt your local database. Lost passwords cannot be recovered.', 'aria-label': 'Enter Vault Password' });
        const btnUnlock = mkEl('button', { class: 'gae-btn', style: 'padding:4px 10px !important; border-radius:6px !important;', title: 'Unlock Vault', 'aria-label': 'Submit password to unlock local vault database' }, ['🔓']);

        btnUnlock.onclick = async (e) => {
            e.preventDefault();
            if (!vi.value || vi.value === '********') return sysLog("Please enter a valid vault password.", "#ff9933");
            const success = await unlockVault(vi.value);
            if (success) {
                vi.value = '********';
                btnUnlock.replaceChildren('✅');
            } else {
                vi.value = '';
                btnUnlock.replaceChildren('❌');
                setTimeout(() => btnUnlock.replaceChildren('🔓'), 2000);
            }
        };
        viWrap.append(vi, btnUnlock);
        ...
        apiGrid.append(selAgent, kiWrap, viWrap, S.nodes.selModel, selFocus, btnProbe);
        ...
        cards.forEach(c => {
            ...
            if (!c.querySelector('.gae-manual-btn')) {
                const b = mkEl('button', { class: 'gae-manual-btn', title: 'Manually Extract Card', 'aria-label': 'Extract Manual', onclick: async (e) => {
                    ...
                    if (!S.key) return sysLog("Vault is locked. Enter password and unlock first.", "#ff4d4d");
